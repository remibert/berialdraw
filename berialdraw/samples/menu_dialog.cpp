...
Button * MenuDialog::add_choice(const String & item)
{
	Button * button = new Button(m_column);            // Creates a new button in the content area for a menu choice
		button->text(item);                            // Sets button text to the provided item
		...
		button->margin(1);                             // Adds margin around the button
	...
	Label * label = new Label(button);                 // Adds a label inside the button for a right arrow
		...
		label->text_color(Color::LIGHT_BLACK);         // Sets arrow text color to light black
