...
void Pie::test1()
{
	...
	for(uint32_t angle = 0; angle <= 360; angle += 20)
	{
		...
		{
			String test_name;
			test_name.print("test/out/pie1_%d.svg",angle/60);
...
void Pie::test2()
{
	...
	for(int32_t angle=90; angle>= -90; angle -=18)
	{
		...
		String name;
		name.print("test/out/pie2_%d.svg", ++id);
...
void Pie::test3()
{
	...
	UIManager::desktop()->dispatch("test/out/pie3.svg");
}
...
void Pie::test4()
{
	...
	for (int speed = 0; speed <= 240; speed+=20)
	{
		...
		{
			String name;
			name.print("test/out/pie4_%d.svg", ++id);
			UIManager::desktop()->dispatch(name);
		}
		else
		{
