...
public:
		...
		Dialog()  // Initializes m_window as the main application window
	{
		...
		switch_ = new Switch(main_layout);                         // Creates a switch
			...
			switch_->on_track_color(Color::LIGHT_GREEN);           // Change the track color when the switch is checked
			switch_->off_track_color(Color::LIGHT_RED);            // Change the track color when the switch is not checked
			switch_->focusable(false);                             // Remove the focus border
